...
* **Overall Line Coverage**: Minimum 75%
* **Function/Branch Coverage**: Strictly monitored to ensure robust logic validation.
* CI will deliberately fail if coverage drops below the 75% mark.
...
- They should be placed in the `tests/` directory, mirroring the structure of the module they test (e.g., tests for `autoweave/models.py` go in `tests/test_models.py`).

### Mocking Guidelines
- Use `unittest.mock` or `pytest-mock` for mocking external dependencies (e.g., database connections, external APIs like Vertex AI).
- Do not mock internal business logic unless necessary for isolating specific behavior.
- Ensure that integration tests hit real databases (like Redis or Postgres) running in isolated Docker containers when possible, rather than mocking everything.

### Interpreting Coverage
- If coverage drops, check the `htmlcov/index.html` report generated by `make test:coverage`.
- Focus on covering the "red" lines, particularly error handling blocks and edge cases.
- Use `# pragma: no cover` sparingly, and only for lines that are genuinely impossible to reach in a test environment.
